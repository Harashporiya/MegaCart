...
export default function PrintersPage() {
  const [electronicItems, setElectronicItems] = useState<LaptopsItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const laptops = response.data.items.filter(
          (item: LaptopsItem) => item.category === 'Laptops'
        );
        //console.log(speakers)
        setElectronicItems(laptops);
        setLoading(false);
      } catch (error) {
      ...
      <SidebarInset>
        ...
        <div className="p-6">
          ...
          {electronicItems.length === 0 ? (
            <p className="text-gray-500">No Laptops items found.</p>
          ) : (
