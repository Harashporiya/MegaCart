...
export default function PrintersPage() {
  const [electronicItems, setElectronicItems] = useState<GameConsolesItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const GameConsoles = response.data.items.filter(
          (item: GameConsolesItem) => item.category === 'GameConsoles'
        );
        //console.log(smartWatches)
        setElectronicItems(GameConsoles);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching data", error);
        setError("Failed to fetch GameConsoles items");
        setLoading(false);
      }
    };
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 border-b">
          <div className="flex items-center gap-2 px-3">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbLink href="#">GameConsoles</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
        ...
        <div className="p-6">
          <h1 className="text-2xl font-bold mb-6">GameConsoles</h1>

          {electronicItems.length === 0 ? (
            <p className="text-gray-500">No GameConsoles items found.</p>
          ) : (
