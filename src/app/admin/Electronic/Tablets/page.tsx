...
export default function PrintersPage() {
  const [electronicItems, setElectronicItems] = useState<TabletsItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const tablets = response.data.items.filter(
          (item: TabletsItem) => item.category === 'Tablets'
        );
        //console.log(speakers)
        setElectronicItems(tablets);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching data", error);
        setError("Failed to fetch laptops items");
        setLoading(false);
      }
    };
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 border-b">
          <div className="flex items-center gap-2 px-3">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbLink href="#">Tablets</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
        ...
        <div className="p-6">
          <h1 className="text-2xl font-bold mb-6">Tablets</h1>

          {electronicItems.length === 0 ? (
            <p className="text-gray-500">No Tablets items found.</p>
          ) : (
              ...
              {electronicItems.map((item) => (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <span className="text-lg font-bold text-gray-900">Stock Quantity: {item.stockQuantity}</span><br/>
