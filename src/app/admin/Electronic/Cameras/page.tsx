...
export default function PrintersPage() {
  const [electronicItems, setElectronicItems] = useState<CamerasItem[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const Cameras = response.data.items.filter(
          (item: CamerasItem) => item.category === 'Cameras'
        );
        //console.log(smartWatches)
        setElectronicItems(Cameras);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching data", error);
        setError("Failed to fetch cameras items");
        setLoading(false);
      }
    };
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 border-b">
          <div className="flex items-center gap-2 px-3">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbLink href="#">Cameras</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
        ...
        <div className="p-6">
          ...
          {electronicItems.length === 0 ? (
            <p className="text-gray-500">No Cameras items found.</p>
          ) : (
