...
export default function LightingPage() {
    const [lightingItem, setLightingItem] = useState<LightingItem[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const lighting = response.data.items.filter(
                    (item: LightingItem) => item.category === 'Lighting'
                );

                setLightingItem(lighting);
                setLoading(false);
            } catch (error) {
                console.error("Error fetching data", error);
                setError("Failed to fetch lighting items");
                setLoading(false);
            }
        };
    ...
    return (
        <SidebarProvider>
            ...
            <SidebarInset>
                <header className="flex h-16 shrink-0 items-center gap-2 border-b">
                    <div className="flex items-center gap-2 px-3">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbLink href="#">Lighting</BreadcrumbLink>
                                </BreadcrumbItem>
                                <BreadcrumbSeparator />
                                <BreadcrumbItem>
                ...
                <div className="p-6">
                    <h1 className="text-2xl font-bold mb-6">Lighting</h1>

                    {lightingItem.length === 0 ? (
                        <p className="text-gray-500">No Lighting  items found.</p>
                    ) : (
                                ...
                                >
                                    ...
                                    <div className="p-4">
                                        ...
                                        <div className="flex justify-between items-center">
                                            ...
                                            <span className="text-sm text-gray-500">Head style: {item.headStyle}</span>
                                        </div>
                                         <span className="text-sm text-gray-500">Room Type: {item.roomType}</span>
