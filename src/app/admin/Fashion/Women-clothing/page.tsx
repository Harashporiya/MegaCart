  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const womenClothingItems = response.data.items.filter(
          (item: FashionItem) => item.category === 'WomensClothing'
        );
        
        setFashionItems(womenClothingItems);
        setLoading(false);
      } catch (error) {
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 border-b">
          <div className="flex items-center gap-2 px-3">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbLink href="#">Women's Clothing</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
        ...
        <div className="p-6">
          <h1 className="text-2xl font-bold mb-6">Women's Clothing</h1>

          {fashionItems.length === 0 ? (
            <p className="text-gray-500">No women's clothing items found.</p>
          ) : (
