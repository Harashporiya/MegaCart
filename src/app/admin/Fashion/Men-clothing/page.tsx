  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const menClothingItems = response.data.items.filter(
          (item: FashionItem) => item.category === 'MensClothing'
        );
        
        setFashionItems(menClothingItems);
        setLoading(false);
      } catch (error) {
      ...
      <SidebarInset>
        ...
        <div className="p-6">
          ...
          {fashionItems.length === 0 ? (
            <p className="text-gray-500">No men's clothing items found.</p>
          ) : (
