  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const homeDecor = response.data.items.filter(
          (item: HomeKitchenItems) => item.category === 'HOME_DECOR'
        );
       
        setHomekitchenItem(homeDecor);
        setLoading(false);
      } catch (error) {
        console.error("Error fetching data", error);
        setError("Failed to fetch home decoration items");
        setLoading(false);
      }
    };
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        <header className="flex h-16 shrink-0 items-center gap-2 border-b">
          <div className="flex items-center gap-2 px-3">
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem>
                  <BreadcrumbLink href="#">Home Decoration</BreadcrumbLink>
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem>
        ...
        <div className="p-6">
          <h1 className="text-2xl font-bold mb-6"> Home Decoration</h1>

          {homekitchenItem.length === 0 ? (
            <p className="text-gray-500">No Home Decoration  items found.</p>
          ) : (
