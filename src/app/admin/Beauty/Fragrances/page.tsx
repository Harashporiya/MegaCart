...
export default function FragrancesPage() {
    const [fragrancesItem, setFragrances] = useState<FragrancesItem[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const fragrances = response.data.items.filter(
                    (item: FragrancesItem) => item.category === 'Fragrances'
                );
                ...
                setError("Failed to fetch fragrances items");
                setLoading(false);
            }
        };
    ...
    return (
        <SidebarProvider>
            ...
            <SidebarInset>
                <header className="flex h-16 shrink-0 items-center gap-2 border-b">
                    <div className="flex items-center gap-2 px-3">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbLink href="#">Fragrances</BreadcrumbLink>
                                </BreadcrumbItem>
                                <BreadcrumbSeparator />
                                <BreadcrumbItem>
                ...
                <div className="p-6">
                    <h1 className="text-2xl font-bold mb-6">Fragrances</h1>

                    {fragrancesItem.length === 0 ? (
                        <p className="text-gray-500">No fragrances  items found.</p>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {fragrancesItem.map((item) => (
                                <div
                                    key={item.id}
                                ...
                                >
                                    <div className="relative w-[400px] flex items-center ml-[70px] h-[330px]">
                                        <Image
                                            src={item.image[selectedImages[item.id] || 7]}
                                            alt={item.name}
                                            fill
