...
export default function MakeupPage() {
    const [makeupItem, setMakeupItem] = useState<MakeupItem[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const makeup = response.data.items.filter(
                    (item: MakeupItem) => item.category === 'Makeup'
                );

                setMakeupItem(makeup);
                setLoading(false);
            } catch (error) {
                console.error("Error fetching data", error);
                setError("Failed to fetch makeup items");
                setLoading(false);
            }
        };
    ...
    return (
        <SidebarProvider>
            ...
            <SidebarInset>
                ...
                <div className="p-6">
                    <h1 className="text-2xl font-bold mb-6">Makeup</h1>

                    {makeupItem.length === 0 ? (
                        <p className="text-gray-500">No makeup  items found.</p>
                    ) : (
