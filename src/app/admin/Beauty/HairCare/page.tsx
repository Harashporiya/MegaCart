...
export default function HairCarePage() {
    const [hairCareItem, setHairCare] = useState<HairCareItem[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const hairCare = response.data.items.filter(
                    (item: HairCareItem) => item.category === 'Hair_Care'
                );

                setHairCare(hairCare);
                setLoading(false);
            } catch (error) {
                console.error("Error fetching data", error);
                setError("Failed to fetch hair care items");
                setLoading(false);
            }
        };
    ...
    return (
        <SidebarProvider>
            ...
            <SidebarInset>
                <header className="flex h-16 shrink-0 items-center gap-2 border-b">
                    <div className="flex items-center gap-2 px-3">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbLink href="#">Hair Care</BreadcrumbLink>
                                </BreadcrumbItem>
                                <BreadcrumbSeparator />
                                <BreadcrumbItem>
                ...
                <div className="p-6">
                    <h1 className="text-2xl font-bold mb-6">Hair Care</h1>

                    {hairCareItem.length === 0 ? (
                        <p className="text-gray-500">No hair care  items found.</p>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {hairCareItem.map((item) => (
                                <div
                                    key={item.id}
                                ...
                                >
                                    <div className="relative w-[400px] flex items-center ml-[70px] h-[330px]">
                                        <Image
                                            src={item.image[selectedImages[item.id] || 15]}
                                            alt={item.name}
                                            fill
