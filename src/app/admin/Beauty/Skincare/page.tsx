...
export default function SkincarePage() {
    const [skincareItem, setskincareItem] = useState<SkincareItem[]>([]);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const skincare = response.data.items.filter(
                    (item: SkincareItem) => item.category === 'Skincare'
                );

                setskincareItem(skincare);
                setLoading(false);
            } catch (error) {
                console.error("Error fetching data", error);
                setError("Failed to fetch skincare items");
                setLoading(false);
            }
        };
    ...
    return (
        <SidebarProvider>
            ...
            <SidebarInset>
                <header className="flex h-16 shrink-0 items-center gap-2 border-b">
                    <div className="flex items-center gap-2 px-3">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                <BreadcrumbItem>
                                    <BreadcrumbLink href="#">Skincare</BreadcrumbLink>
                                </BreadcrumbItem>
                                <BreadcrumbSeparator />
                                <BreadcrumbItem>
                ...
                <div className="p-6">
                    <h1 className="text-2xl font-bold mb-6">Skincare</h1>

                    {skincareItem.length === 0 ? (
                        <p className="text-gray-500">No Skincare  items found.</p>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {skincareItem.map((item) => (
                                <div
                                    key={item.id}
                                ...
                                >
                                    <div className="relative w-full h-64">
                                        <Image
                                            src={item.image[selectedImages[item.id] || 8]}
                                            alt={item.name}
                                            fill
